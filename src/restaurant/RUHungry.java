...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for (int i = 0; i < arraySize; i++) {
            MenuNode headMenuNode = null;
            category = StdIn.readLine();
            int numberOfDishes = Integer.parseInt(StdIn.readLine());
            categoryVar[i] = category;
            for (int a = 0; a < numberOfDishes; a++) {
                String dishName = StdIn.readLine();
                int numberOfingredientID = Integer.parseInt(StdIn.readString());
                int[] ingredientID = new int[numberOfingredientID];
                for (int j = 0; j < numberOfingredientID; j++) {
                    ingredientID[j] = StdIn.readInt();
                }
                Dish testDish = new Dish(category, dishName, ingredientID);
                MenuNode meen = new MenuNode(testDish, null);
                meen.setNextMenuNode(headMenuNode);
                headMenuNode = meen;
                menuVar[i] = headMenuNode;
    ...
    public void addStockNode(StockNode newNode) {
        ...
        if (stockVar[index] == null) {
            ...
            StockNode x = stockVar[index];
            stockVar[index] = newNode;
            newNode.setNextStockNode(x);
        }
        // WRITE YOUR CODE HERE
    }
    ...
    public void updateStock(String ingredientName, int ingredientID, int stockAmountToAdd) {
        if (ingredientName != null) {
            ...
            ingredient.getIngredient().updateStockLevel(stockAmountToAdd);
    ...
    public void updatePriceAndProfit() {
        ...
        for (int i = 0; i < menuVar.length; i++) {
            ...
            while (ptr != null) {
                ...
                double dishPrice = 0;
                double profit = 0;
                Ids = ptr.getDish().getStockID();
                for (int a = 0; a < Ids.length; a++) {

                    StockNode ingredientNode = findStockNode(Ids[a]);
                    if (ingredientNode != null) {
                        ingredientCost += ingredientNode.getIngredient().getCost();
                    }
                }
                dishPrice = ingredientCost * 1.2;
                profit = dishPrice - ingredientCost;
    ...
    public void createStockHashTable(String inputFile) {
        ...
        while (StdIn.hasNextLine()) {
            ...
            Ingredient bruhIngredient = new Ingredient(ingredientID, ingredientName, stockAmount, ingredientCost);
            StockNode bruhStockNode = new StockNode(bruhIngredient, null);
            addStockNode(bruhStockNode);
        }
        printRestaurant();
    ...
    public void addTransactionNode(TransactionData data) { // method adds new transactionNode to the end of LL
        TransactionNode neue = new TransactionNode(data, null);
        if (transactionVar == null) {
            transactionVar = neue;
        } else {
            TransactionNode currNode = transactionVar;
            while (currNode.getNext() != null) {
                currNode = currNode.getNext();
            }
            currNode.setNext(neue);
        }
    ...
    public boolean checkDishAvailability(String dishName, int numberOfDishes) {
        ...
        for (int i = 0; i < id.length; i++) {
            int ingredientStock = findStockNode(id[i]).getIngredient().getStockLevel();
            if (ingredientStock >= numberOfDishes) {
                trueorfalse++;

            } else if (!(ingredientStock >= numberOfDishes)) {
                trueorfalse--;
                status = false;
            }
            if (trueorfalse == -1) {
        ...
        // WRITE YOUR CODE HERE
        if (trueorfalse == id.length) {
            return true;
        }

        return status; // update the return value
    }
    ...
    private String startingDishyName;
    ...
    public void order(String dishName, int quantity) {
        ...
        boolean order = false;
        int storedNumber1 = 0;
        if (startingDishyName == null) {
            startingDishyName = dishName;
        }
        if (!order) {
            for (int i = 0; i < menuVar.length; i++) { // traverses menuVar
                for (MenuNode ptr = menuVar[i]; ptr != null; ptr = ptr.getNextMenuNode()) {
                    double profit = 0;
                    IDs = ptr.getDish().getStockID();
                    Dish dishNameDish = ptr.getDish();
                    if (dishName.equals(dishNameDish.getName())) {
                        if (checkDishAvailability(dishName, quantity) == true) {
                            profit = ptr.getDish().getProfit() * quantity;
                            TransactionData orderTransactionData = new TransactionData("order", dishName,
                                    quantity,
                                    profit,
                                    true);
                            addTransactionNode(orderTransactionData);
                            order = true;
                            startingDishyName = null;
                            for (int a = 0; a < IDs.length; a++) { // traverse ingridents id array
                                if (dishName.equals(dishNameDish.getName())) {
                                    String ingredientName = findStockNode(IDs[a]).getIngredient().getName();
                                    updateStock(ingredientName, IDs[a], -quantity);
                                }
                            }
                        }
                    }
                    if (dishName.equals(dishNameDish.getName())) {
                        String category = dishNameDish.getCategory();
                        Dish startingDish = dishNameDish;
                        int categoryIndex = 0;
                        for (int j = 0; j < categoryVar.length; j++) {
                            if (categoryVar[j].equals(category)) {
                                categoryIndex = j;
                            }
                        }
                        if (!order) {
                            TransactionData orderTransactionData1 = new TransactionData("order",
                                    dishName,
                                    quantity, 0,
                                    false);
                            addTransactionNode(orderTransactionData1);
                            MenuNode lltraverse = menuVar[categoryIndex];
                            MenuNode lltraverse1 = menuVar[categoryIndex];
                            MenuNode headMenuNode = menuVar[categoryIndex];
                            MenuNode lltraverse3 = menuVar[categoryIndex];
                            Dish nextDish = null;
                            while (lltraverse3 != null) {
                                if (lltraverse3 != null) {
                                    lltraverse3 = lltraverse3.getNextMenuNode();
                                    storedNumber1++;
                            ...
                            if (storedNumber1 == 2) {
                                storedNumber = storedNumber1;
                                allVisited = false;
                            }
                            while (nextDish == null) {
                                if (lltraverse1.getDish().equals(startingDish)) {
                                    nextDish = lltraverse1.getNextMenuNode().getDish();
                                    break;
                                }
                                if (lltraverse1 != null) {
                                    lltraverse1 = lltraverse1.getNextMenuNode();
                                }
                                if (lltraverse1.getNextMenuNode() == null) {
                                    storedNumber = 0;
                                    allVisited = true;
                                    nextDish = headMenuNode.getDish();
                                    break;
                                }
                            }
                            if (storedNumber1 > 2) {
                            ...
                            if (!order && storedNumber == 2) {
                                startingDishyName = dishName;
                            }
                            while (!order && !allVisited) {
                                if (nextDish.getName().equals(startingDishyName)) {
                                    break;
                                }
                                while (lltraverse != null && !lltraverse.getDish().equals(nextDish)) {
                                    lltraverse = lltraverse.getNextMenuNode();
                                }
                                {
                                    if (lltraverse != null && lltraverse.getDish().equals(nextDish)) {
                                        order(lltraverse.getDish().getName(), quantity);
                                        break;
                                    }
                                    if (lltraverse == null) {
                                        lltraverse = headMenuNode;
                                    }
                                }
                            }
                        }
    ...
    public void donation(String ingredientName, int quantity) {
        ...
        int ID = findStockNode(ingredientName).getIngredient().getID();
        int stocklevel = findStockNode(ingredientName).getIngredient().getStockLevel();
        if (profit() > 50 && stocklevel >= quantity) {
            success = true;
        } else {
            success = false;
        }
        TransactionData donate = new TransactionData("donation", ingredientName, quantity, 0, success);
    ...
    public void restock(String ingredientName, int quantity) {
        ...
        boolean success = false;
        int ID = findStockNode(ingredientName).getIngredient().getID();
        double ingredientCost = findStockNode(ingredientName).getIngredient().getCost();
        double costOfRestocking = ingredientCost * quantity;
        if (profit() >= costOfRestocking) {
            stockable = -costOfRestocking;
            success = true;
        } else {
            stockable = 0;
            success = false;
        }
        TransactionData stock = new TransactionData("restock", ingredientName, quantity, stockable, success);
        addTransactionNode(stock);
        if (success && stockable == -costOfRestocking) {
            updateStock(ingredientName, ID, quantity);
        }
